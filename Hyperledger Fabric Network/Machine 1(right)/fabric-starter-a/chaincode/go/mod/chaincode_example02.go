...
func (s *SmartContract) publishRequest(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	val,ok,err := cid.GetAttributeValue(stub, "type")
		...
		if ok == false {
		return shim.Error("Attribute VAlue Not found for user")
		}
		if val == "Patient" {
		return shim.Error("User is Patient type. Cannot perform function")
		}
    //var Status = "pending"
...
func (s *SmartContract) response(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	err := cid.AssertAttributeValue(stub, "type", "Patient")
		if err!= nil {
		return shim.Error(err.Error())
		}
